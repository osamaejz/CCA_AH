$(function() {
    ...
    DOMstrings.stepsForm.addEventListener('click', e => {
        ...
        var i = 0;
        var flag = true;
        parent_fieldset.find('.r').each( function(){
            next_step = false;
            
            var radioName = $(this).attr('name');
            if ($('input[name='+ radioName +']:checked').length) {
                // at least one of the radio buttons was checked
                $('.' + radioName).attr('style','display: none;');
           }
            else {
                flag = false;
                // no radio button was checked
                $('.' + radioName).attr('style','display: block;');
                if(i==0){
                    $('html, body').animate({
                        scrollTop: $('.' + radioName).focus().offset().top - 250
                    }, 500);
                }
                i = 1;
            }
